...
Provides configurable logging with Rich formatting for all color-scheme projects.
"""
...
def setup_logging(
    ...
    Args:
        ...
        logger_name: Base name for the logger namespace
    """
    global _console
...
def get_logger(name: str, logger_namespace: str = "colorscheme") -> logging.Logger:
    """Get a logger for a module.

    Args:
        name: Logger name (usually __name__)
        logger_namespace: Base namespace for logger (default: colorscheme)
    ...
    if not name.startswith(logger_namespace):
        full_name = f"{logger_namespace}.{name}"
    else:
        full_name = name
